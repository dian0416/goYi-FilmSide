...
open class LLCycleScrollView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate {
    ...
    open var winnerTitles: Array<String> = [] {
        didSet {
            if winnerTitles.count > 0 {
                if imagePaths.count == 0 {
                    imagePaths = winnerTitles
                }
            }
        }
    }
    ...
    open class func llCycleScrollViewWithFrame(_ frame: CGRect, imageURLPaths: Array<String>? = [], titles:Array<String>? = [], didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> LLCycleScrollView {
        ...
        let maskPath = UIBezierPath.init(arcCenter: CGPoint.init(x: llcycleScrollView.bounds.size.width/2, y: -llcycleScrollView.bounds.size.height-10), radius: llcycleScrollView.bounds.size.width + 30, startAngle: 0, endAngle: llcycleScrollView.bounds.size.width, clockwise: true)

        let maskLayer = CAShapeLayer()
        maskLayer.frame = llcycleScrollView.bounds
        maskLayer.path = maskPath.cgPath
        llcycleScrollView.layer.mask = maskLayer
...
//        collectionView.backgroundColor = collectionViewBackgroundColor
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isOnlyTitle && titles.count > 0{
            ...
            cell.winnerTitle = winnerTitles[itemIndex]
        }else{
            // Mode
