...
extension UIResponder {
    ...
    func pleaseWaitWithImages(_ imageNames: Array<UIImage>, timeInterval: CGFloat) {
        HUDNotice.wait(imageNames, timeInterval: timeInterval)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    // api changed from v3.3
    func noticeTop(_ text: String, autoClearTime: CGFloat = 1) {
        HUDNotice.noticeOnStatusBar(text, autoClearTime: autoClearTime)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    
    // new apis from v3.3
    func noticeSuccess(_ text: String, autoClearTime: CGFloat = 1.5) {
        HUDNotice.showNoticeWithText(NoticeType.success, text: text, autoClearTime: autoClearTime)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    func noticeError(_ text: String, autoClearTime: CGFloat = 3) {
        HUDNotice.showNoticeWithText(NoticeType.error, text: text, autoClearTime: autoClearTime)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    func noticeInfo(_ text: String, autoClearTime: CGFloat = 3) {
        HUDNotice.showNoticeWithText(NoticeType.info, text: text, autoClearTime: autoClearTime)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    
    // old apis
    func successNotice(_ text: String, autoClear: Bool = true) {
        HUDNotice.showNoticeWithText(NoticeType.success, text: text, autoClearTime: 3)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    func errorNotice(_ text: String, autoClear: Bool = true) {
        HUDNotice.showNoticeWithText(NoticeType.error, text: text, autoClearTime: 3)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    func infoNotice(_ text: String, autoClear: Bool = true) {
        HUDNotice.showNoticeWithText(NoticeType.info, text: text, autoClearTime: 3)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    func notice(_ text: String, type: NoticeType, autoClear: Bool, autoClearTime: CGFloat = 3) {
        HUDNotice.showNoticeWithText(type, text: text, autoClearTime: autoClearTime)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    func pleaseWait() {
        HUDNotice.wait()
    ...
    func noticeOnlyText(_ text: String) {
        HUDNotice.showText(text, autoClearTime: 1.5)
        UIApplication.shared.keyWindow?.isUserInteractionEnabled = false
    }
    func clearAllNotice() {
        HUDNotice.clear()
...
class HUDNotice: NSObject {
    ...
    static func noticeOnStatusBar(_ text: String, autoClearTime: CGFloat?) {
        ...
        if autoClearTime != nil {
            let selector = #selector(HUDNotice.hideNotice(_:))
            self.perform(selector, with: window, afterDelay: TimeInterval(autoClearTime!))
        }
    }
    static func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: CGFloat = 0) {
        ...
        if imageNames.count > 0 {
            if imageNames.count > timerTimes {
                ...
                timer.scheduleRepeating(deadline: DispatchTime.now(), interval: DispatchTimeInterval.milliseconds(Int(timeInterval)))
    ...
    static func showText(_ text: String, autoClearTime: CGFloat?) {
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        mainView.layer.cornerRadius = 12
    ...
    static func showNoticeWithText(_ type: NoticeType,text: String, autoClearTime: CGFloat?) {
        ...
        switch type {
        case .success:
            image = HUDNoticeSDK.imageOfCheckmark
        case .error:
            image = HUDNoticeSDK.imageOfCross
        case .info:
            image = HUDNoticeSDK.imageOfInfo
        }
    ...
    // fix https://github.com/johnlui/HUDNotice/issues/2
...
class HUDNoticeSDK {
    ...
    class var imageOfCheckmark: UIImage {
        ...
        HUDNoticeSDK.draw(NoticeType.success)
    ...
    class var imageOfCross: UIImage {
        ...
        HUDNoticeSDK.draw(NoticeType.error)
    ...
    class var imageOfInfo: UIImage {
        ...
        HUDNoticeSDK.draw(NoticeType.info)
