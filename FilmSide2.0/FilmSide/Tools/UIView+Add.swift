...
//MARK:Layer
     ...
     * 默认阴影范围为size(1,1)
     - parameter color:
     */
    func shadow(color:UIColor = UIColor.gray,shadowOpacity:Float = 0.5,shadowRadius:CGFloat = 2,shadowOffset:CGSize = CGSize(width:1, height:1)) {
...
//MARK:ACTION
    func addAction(closure:@escaping ()->()){
        let tap = UITapGestureRecognizer(target: self, action: #selector(UIView.tapClick))
